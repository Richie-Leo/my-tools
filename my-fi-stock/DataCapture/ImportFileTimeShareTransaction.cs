...
namespace Pandora.Invest.DataCapture
{
	...
	public static class ImportFileTimeShareTransaction
	{
		...
		public static void DoImportFile(Database db, string file){
            ...
            {
                TimeshareBulkInserter<object[]> bi = new TimeshareBulkInserter<object[]>(db, 
                    string.Format("sto_trade_{0}", code), 500
                );
                string line;
    ...
    public class TimeshareBulkInserter<T> : BulkInserter<T>
    {
        public TimeshareBulkInserter(Database db, string tableName, int batchSize) : base(db, tableName, new string[] {
                "sto_id", "trade_time", "price", "qty", "match_num", "type"
            }, batchSize)
        { }

        public override BulkInserter<T> Push(T obj)
        {
            object[] e = obj as object[];
