    ...
    public class ImpShareHoldersNumWorker : MThreadWorker<Stock>{
        ...
        public override void Do(MThreadContext context, Stock stock){
            ...
            while(retries<MAX_RETRIES){
                ...
                Info("http-get:" + entities.Count + ", db-update:" + insertedRows + ", http-time=" +
                    (ts1.TotalMilliseconds/1000.0).ToString("f2") + "s, parse-time=" + 
                    (ts2.TotalMilliseconds/1000.0).ToString("f2") + "s, db-time=" + 
                    (ts3.TotalMilliseconds/1000.0).ToString("f2"));
                break;
            }
            if(retries>=MAX_RETRIES){
                Info(stock.StockCode + ": 3 retries failed, ignored");
            }
        }
